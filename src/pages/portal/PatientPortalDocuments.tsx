...
export default function PatientPortalDocuments() {
  ...
  if (!settings.navigation.documents) {
    return (
      <PatientPortalDisabledState
        title="Documentos temporariamente ocultos"
        description="Este modulo foi retirado da navegacao pelo responsavel do portal."
      />
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {settings.documents.exams || settings.documents.reports ? (
        <div className="grid gap-4 lg:grid-cols-2">
          {settings.documents.exams ? (
            <Card className="rounded-3xl border-border/60">
            ...
            {materialRecords.length === 0 ? (
              ...
              materialRecords.map((record) => (
                ...
                >
                  ...
                  {settings.documents.nextSteps && record.next_steps?.trim() ? (
                  ...
                  {settings.documents.attachments && (record.files?.length ?? 0) > 0 ? (
                  ...
                  {!settings.documents.nextSteps && !settings.documents.attachments ? (
                    <p className="mt-3 text-sm text-muted-foreground">
                      Os detalhes deste material foram ocultados pelo admin.
                    </p>
                  ) : null}
                </div>
              ))
      ...
      !settings.documents.materials ? (
        <Card className="rounded-3xl border-dashed border-border/80">
          <CardContent className="p-6 text-sm text-muted-foreground">
            O modulo esta ativo, mas exames, relatorios e materiais foram ocultados pelo admin.
          </CardContent>
        </Card>
      ) : null}
    </div>
