...
import { QuickMessageActions } from "@/components/admin/messages/QuickMessageActions";
  ...
  [
    {
      icon: Target,
      label: "Objetivo",
      val: notes.goal ? GOAL_LABELS[notes.goal] || notes.goal : null,
    },
    {
      icon: Salad,
      label: "Restricoes",
      val: notes.restrictions
        ? RESTRICT_LABELS[notes.restrictions] || notes.restrictions
        : null,
    },
    { icon: HelpCircle, label: "Alergias", val: notes.allergies },
    { icon: Heart, label: "Condicoes", val: notes.healthConditions },
    { icon: Pill, label: "Medicamentos", val: notes.medications },
    { icon: User, label: "Rotina", val: notes.rotina },
    { icon: Heart, label: "Sintomas", val: notes.sintomas },
    { icon: Pill, label: "Suplementacao", val: notes.suplementacao },
    { icon: Salad, label: "Exames previos", val: notes.examesPrevios },
    {
      icon: User,
      label: "Acomp. ant.",
      val:
        notes.hadNutritionist === "sim"
          ? "Sim"
          : notes.hadNutritionist
            ? "Nao"
            : null,
    },
    { icon: UserPlus, label: "Historico com nutri", val: notes.historicoNutri },
    {
      icon: HelpCircle,
      label: "Como chegou",
      val: notes.howFound ? FOUND_LABELS[notes.howFound] || notes.howFound : null,
    },
    { icon: Pencil, label: "Observacoes", val: notes.observacoes },
  ].filter((row) => row.val);
...
export const BookingPatientPanel = ({
    ...
    return (
      <div
        className={
          isMobile
            ? "p-4 space-y-3"
            : "flex flex-col h-full overflow-y-auto p-4 space-y-3"
        }
      >
        <div className="flex items-center justify-between">
          <p
            className={
              isMobile
                ? "text-xs font-bold uppercase tracking-widest text-muted-foreground/50"
          ...
          <button
            ...
            className={
              ...
              " rounded flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
            }
          >
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            onChange={(event) => setEditRestrictions(event.target.value)}
            className={selectClass}
          >
            <option value="">-</option>
        ...
        <div className="space-y-1">
          ...
          <Input
            value={editHealth}
            onChange={(event) => setEditHealth(event.target.value)}
            className={inputClass}
          />
        </div>
        <div className="space-y-1">
          ...
          <Input
            value={editMeds}
            onChange={(event) => setEditMeds(event.target.value)}
            className={inputClass}
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          <select
            value={editHadNutri}
            onChange={(event) => setEditHadNutri(event.target.value)}
            className={selectClass}
          >
            <option value="">-</option>
        ...
        <div className="space-y-1">
          ...
          <select
            value={editHowFound}
            onChange={(event) => setEditHowFound(event.target.value)}
            className={selectClass}
          >
            <option value="">-</option>
        ...
        <div className={isMobile ? "flex gap-2 pt-1 pb-4" : "flex gap-2 pt-1"}>
          <button
            ...
            className={
              ...
              " flex-1 rounded-lg bg-primary text-primary-foreground font-semibold hover:bg-primary/90 transition-colors disabled:opacity-50 flex items-center justify-center gap-1.5"
            }
          >
            {saving ? (
              <Loader2
          ...
          <button
            ...
            className={
              (isMobile ? "h-10 text-sm" : "h-8 text-xs") +
              " flex-1 rounded-lg border border-border text-muted-foreground font-medium hover:text-foreground hover:bg-muted transition-colors"
            }
          >
            Cancelar
          </button>
  ...
  const contactBoxClass = isMobile
    ? "bg-muted/30 rounded-lg p-3 space-y-2.5"
    : "space-y-2";
  const clinicalBoxClass = isMobile
  ...
  return (
    <div className={isMobile ? "p-4 space-y-5" : "p-4 space-y-5 overflow-y-auto"}>
        ...
        <div className={contactBoxClass}>
            ...
            <div className={isMobile ? "flex items-start gap-2.5" : "flex items-start gap-2"}>
              <Mail
                className={
                  ...
                  " text-muted-foreground/40 shrink-0 mt-px"
                }
              />
              <span
                className={
                  (isMobile ? "text-sm" : "text-xs") +
                  " text-foreground break-all leading-snug"
                }
              >
                {booking.client_email}
              </span>
          ...
          {!isMobile &&
            (booking.patient_id ? (
              ...
              >
                {creatingPatient === booking.booking_group_id ? (
      ...
      <div className={isMobile ? "space-y-3" : "space-y-2.5"}>
        <QuickMessageActions booking={booking} compact={!isMobile} />
      </div>
            ...
            {clinicalRows(notes).map(({ icon: Icon, label, val }) =>
              isMobile ? (
                ...
                <div key={label}>
                  ...
                  <p className="text-xs font-medium text-foreground leading-snug pl-3.5">
                    {val}
                  </p>
                </div>
              ),
