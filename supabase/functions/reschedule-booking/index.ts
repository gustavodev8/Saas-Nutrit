...
serve(async (req) => {
  ...
  try {
    ...
    if (!booking_id || !new_date || !new_time || !client_email) {
      return jsonResponse({ error: "Campos obrigatórios ausentes." }, 400, corsHeaders);
    }
    ...
    const updateRes = await fetch(`${auth.supabaseUrl}/rest/v1/bookings?id=eq.${encodeURIComponent(String(booking_id))}`, {
      method: "PATCH",
      headers: {
    ...
    if (resendKey) {
      ...
      const safeClientName = escapeHtml(client_name || "cliente");
      const safePlanName = escapeHtml(plan_name || "Consulta");
            ...
            <tr><td align="center">
              <table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.06);">
                ...
                <tr><td style="padding:32px 40px;">
                  ...
                  <p style="margin:0 0 24px;font-size:14px;color:#6b7280;">Olá, ${safeClientName} — sua data foi alterada.</p>
                  <div style="background:#f9fafb;border-radius:8px;padding:20px 24px;margin-bottom:16px;">
                    ...
                    <p style="margin:0;font-size:16px;font-weight:600;color:#111827;">${safePlanName}</p>
                  </div>
                ...
                <tr><td style="padding:0 40px 32px;">
                  ...
                  <p style="margin:0;font-size:12px;color:#d1d5db;text-align:center;">${safeBrand} &nbsp;·&nbsp; Este é um email automático</p>
                </td></tr>
              </table>
            </td></tr>
          </table>
